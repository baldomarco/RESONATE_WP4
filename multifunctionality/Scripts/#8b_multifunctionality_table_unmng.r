...
dataroot <- "I:/iLand/2024/output/20241105/UNMNG_hist/"
...
for (i in (1:length(all_v)))  {
  ...
  landscape.area<-landscape$area[1]                                             # CREATE THE VARIABLE FOR LANDSCAPE AREA
  #-----------------------------------------------------------------------------
  # CREATE SHANNON VARIABLE FROM STAND (alpha diversity)
  # Within your loop
  stand <- stand %>%
...
write.csv(multifunctionality_table, paste0(dataroot,"Multifunctionality_Table_UNMNG_hist.csv"), row.names = TRUE)
write.csv(H, paste0(dataroot,"Multifunctionality_Table_H_UNMNG_hist.csv"), row.names = TRUE)
write.csv(H_avg, paste0(dataroot,"Multifunctionality_Table_H_avg_UNMNG_hist.csv"), row.names = TRUE)
...
write.csv(summary_table, paste0(dataroot, "Multifunctionality_Summary_Table_UNMNG_hist.csv"), row.names = TRUE)
...
write.csv(transposed_summary_table, paste0(dataroot, "Multifunctionality_Summary_Table_UNMNG_hist_transposed.csv"), row.names = TRUE)
