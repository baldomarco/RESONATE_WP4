...
# Create a survival object treating 49 as the censoring point
# status = 0 means censored (didn't recover), 1 = recovered
df <- df |>
  mutate(
    rt = if_else(is.infinite(rt), 50, rt),
    status = if_else(rt > 49, 0, 1),
    rt = pmin(rt, 50),  # cap at censoring point
    rt_surv = Surv(rt, status)
  )

# Tobit model via survreg (equivalent to censored regression)
tobit_mod <- survreg(Surv(rt, status) ~ mgm + rcp + windcase,
