...
var finalcut_cc = {
  ...
  onEvaluate: function() {
    ...
    let nlarge = trees.load('dbh>=30');
    trees.randomize();
    trees.filter('incsum(1) < ' + (nlarge - 10)); // Corrected the parentheses // if you have 100 trees >30, this would be then: incsum(1)<90 -> 10 trees would be excluded from filtering
    trees.filter("dbh>5");
    trees.filter("age>0");
