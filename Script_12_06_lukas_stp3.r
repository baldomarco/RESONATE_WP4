...
BAU_V1 <-paste0(dataroot,"BAU_WP4_20231223.sqlite")
BIOEC_V2 <- paste0(dataroot,"BIOECONOMY_WP4_20221223.sqlite")
CNS_V2 <- paste0(dataroot,"CONSERVATION_WP4_20231223.sqlite")
ADP_V2 <- paste0(dataroot,"ADAPTATION_WP4_20231223.sqlite")
  
# ----------------------------------------------------------- READ IN THE DATA:

sqlite.driver <- dbDriver("SQLite")
db1 <- dbConnect(sqlite.driver, dbname = ADP_V2)
...
barplot(toplot, 
        col=colors, 
        main="Whole landscape thinnings harvests salvage m3/ha/year", 
        ylim=c(0,40), 
...
# sid  7095 stp1 - 8098 stp2 - 7858 stp3 - 36853 stp4 - 36857 stp5  |  
#      13872 stp1 - 13802 stp2 - 13737 stp3 - 13755 stp4 - 13655 stp4 - 13675 stp5 |
#      19555 stp1 - 19629 stp2 - 19558 stp3 - 19608 stp4 - 19559 stp5 - 19601 stp5
# rid 3241 -  4990 - 5551 - 1159 - 3530 | 
#     3231 - 3464 - 4219 - 4151 - 4054 - 5106 |
#     4172 - 4453 - 4172 - 5025 - 4172 - 4316

mystands<-c(7095, 8098, 7858, 36853, 36857, 11413)
myrus<-c(3241, 4990, 5551, 1159, 3530, 5144)

mystands<-c(13872, 13802, 13737, 13755, 13655, 13675)
myrus<-c(3231, 3436, 4219, 4151, 4054, 5106)
...
for (cases in 1:length(mystands)) {
  ...
  outputfilename<-"ADP_management_test_20231222_1st_group"
  ...
  # First plot
  plot(abestand1$year, abestand1[, 6], typ = "l", lwd = 2, ylab = names[6], xlab = "years",
       main = paste("standid=", abestand1$standid[1], ",", names[1]))
  ...
  # Second plot
  par(new = TRUE)
  barplot(t(removals[, -1]), xlab = "", ylab = "", xaxt = "n", yaxt = "n", ylim = current_par[3:4], col = colors, width = 0.1)
  
  yticks <- pretty(current_par[3:4])
  ...
  for (i in 7:ncol(abestand)) {
    plot(abestand1$year,abestand1[,i], typ="l",lwd=1, ylab=names[i], xlab="years",main=paste("standid=",abestand1$standid[1],",",names[i] ))
    
  }
  
  # tending<-which(abestand.removal1$activity=="_sw"&abestand.removal1$volumeFinal==0) old version from Laura#
  # tending <- which(grepl("_sw", abestand.removal1$activity) | grepl("thinning", abestand.removal1$activity)) # new version from Marco good but is transforming everything in tending (thinning and salvagin include)
  
  tending <- which(grepl("_sw", abestand.removal1$activity) & grepl("thinning", abestand.removal1$activity))
  ...
  for (nn in 1:number.of.activites){
    g.age<-g.age+geom_vline(xintercept =abestand.removal1$year[nn], col="limegreen", size=0.5,lty=2)+annotate(geom="text", x=0.95*abestand.removal1$year[nn], y=80, label=abestand.removal1$activity[nn], color="red", angle=90)
  }
  ...
  for (nn in 1:number.of.activites){
    g.dbh<-g.dbh+geom_vline(xintercept =abestand.removal1$year[nn], col="limegreen", size=0.5,lty=2)+annotate(geom="text", x=0.95*abestand.removal1$year[nn], y=50, label=abestand.removal1$activity[nn], color="red", angle=90)
  }
...
selected_numbers <- sample(numbers, 4)
